...
router.post("/recommendation", async (req, res) => {
  ...
  try {
    ...
    const recommendation = await Recommendation.findOne({
      destination: { $regex: new RegExp(destination.trim(), "i") }, // case-insensitive search
    });
    ...
    // Select hotels (fallback to first hotel if none match)
    const selectedHotels = hotels.length > 0 ? hotels : [recommendation.hotels[0]];

    // Organize attractions into daily itinerary
    const attractions = recommendation.attractions;
    const dailyAttractions = [];
    const maxAttractionsPerDay = 2; // Change this if you want more/less attractions per day
